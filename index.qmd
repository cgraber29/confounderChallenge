...
k <- 1.5  # controls sensitivity
patients_df$prob_duck <- 1 / (1 + exp(-k * patients_df$severity))

# Random assignment: if random < prob_duck → Doc Duck (id=0), else Doc Dreamy (id=1)
patients_df$doctor_id <- as.integer(runif(n_patients) >= patients_df$prob_duck)

# Assign doctor names based on id
patients_df$doctor_name <- ifelse(patients_df$doctor_id == 1, 'Doc Dreamy', 'Doc Duck')

# Assign surgical goodness:
# Doc Duck: Normal(0.5, 1), Doc Dreamy: Normal(-0.4, 1)
patients_df$surgicalGoodness <- ifelse(
  patients_df$doctor_name == 'Doc Duck',
  rnorm(n_patients, 0.5, 1),
  rnorm(n_patients, -0.4, 1)
)

# Calculate post-surgical severity: initial severity - surgical goodness + 3
patients_df$post_surgical_score <- patients_df$severity - patients_df$surgicalGoodness + 3

# Save to CSV, excluding prob_duck and surgicalGoodness columns
write.csv(
  patients_df[, c('patient', 'severity', 'doctor_id', 'doctor_name', 'post_surgical_score')],
  'patients_data.csv',
  row.names = FALSE
)
...
@fig-plot-outcomes presents a scatter plot showing post-surgical symptom scores by patient number, with different markers and colors for each doctor. The figure includes scatter points for each patient (Doc Dreamy: circles, Doc Duck: triangles), horizontal dashed lines showing the mean for each doctor, and text annotations displaying the mean values with shape symbols (○ for Doc Dreamy, △ for Doc Duck). The visualization uses colorblind-friendly colors (blue for Doc Dreamy, coral/red-orange for Doc Duck) and professional styling with clear labels, grid, and legend.
...
# Separate data by doctor and calculate means
...
# Create the plot
p <- ggplot(patients_df, aes(x = patient, y = post_surgical_score, 
                             color = doctor_name, shape = doctor_name)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_hline(yintercept = dreamy_mean, color = '#4E79A7', 
            linetype = 'dashed', linewidth = 1, alpha = 0.8) +
  geom_hline(yintercept = duck_mean, color = '#E15759', 
            linetype = 'dashed', linewidth = 1, alpha = 0.8) +
  scale_color_manual(
    name = 'Doctor',
  ...
  annotate('text', x = max(patients_df$patient) * 0.98, y = dreamy_mean,
           label = paste0('○ ', sprintf('%.2f', dreamy_mean)),
           color = '#4E79A7', size = 3.5, fontface = 'bold',
           hjust = 1, vjust = 0.5) +
  annotate('text', x = max(patients_df$patient) * 0.98, y = duck_mean,
           label = paste0('△ ', sprintf('%.2f', duck_mean)),
...
patients_randomized_df <- data.frame(
  patient = 1:n_patients_randomized,
  severity = rnorm(n_patients_randomized, 0, 1)
)

# Randomly assign doctors: 50/50 chance, independent of severity
# This breaks the confounding relationship
patients_randomized_df$doctor_id <- sample(c(0, 1), n_patients_randomized, replace = TRUE)

# Assign doctor names based on id
patients_randomized_df$doctor_name <- ifelse(
  patients_randomized_df$doctor_id == 1, 'Doc Dreamy', 'Doc Duck'
)

# Assign surgical goodness:
# Doc Duck: Normal(0.5, 1), Doc Dreamy: Normal(-0.4, 1)
# Same true effect as before - Doc Duck is better
patients_randomized_df$surgicalGoodness <- ifelse(
  patients_randomized_df$doctor_name == 'Doc Duck',
  rnorm(n_patients_randomized, 0.5, 1),
  rnorm(n_patients_randomized, -0.4, 1)
)

# Calculate post-surgical severity: initial severity - surgical goodness + 3
patients_randomized_df$post_surgical_score <- 
  patients_randomized_df$severity - patients_randomized_df$surgicalGoodness + 3

# Save to CSV, excluding surgicalGoodness column
write.csv(
  patients_randomized_df[, c('patient', 'severity', 'doctor_id', 'doctor_name', 'post_surgical_score')],
  'patients_data_randomized.csv',
  row.names = FALSE
)
...
@fig-plot-outcomes-randomized presents a scatter plot similar to Figure 1, but for the randomized data. This figure demonstrates how randomization breaks the confounding relationship. It uses the same styling as Figure 1 (markers, colors, mean lines, annotations), with the title indicating randomized assignment, maintaining the same colorblind-friendly color scheme and professional styling.
...
# Separate randomized data by doctor and calculate means
...
# Create the plot
p <- ggplot(patients_randomized_df, aes(x = patient, y = post_surgical_score, 
                                        color = doctor_name, shape = doctor_name)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_hline(yintercept = dreamy_mean_rand, color = '#4E79A7', 
            linetype = 'dashed', linewidth = 1, alpha = 0.8) +
  geom_hline(yintercept = duck_mean_rand, color = '#E15759', 
            linetype = 'dashed', linewidth = 1, alpha = 0.8) +
  scale_color_manual(
    name = 'Doctor',
    values = c('Doc Dreamy' = '#4E79A7', 'Doc Duck' = '#E15759'),
  ...
  annotate('text', x = max(patients_randomized_df$patient) * 0.98, y = dreamy_mean_rand,
           label = paste0('○ ', sprintf('%.2f', dreamy_mean_rand)),
           color = '#4E79A7', size = 3.5, fontface = 'bold',
           hjust = 1, vjust = 0.5) +
  annotate('text', x = max(patients_randomized_df$patient) * 0.98, y = duck_mean_rand,
           label = paste0('△ ', sprintf('%.2f', duck_mean_rand)),
           color = '#E15759', size = 3.5, fontface = 'bold',
           hjust = 1, vjust = 0.5) +
  labs(
    x = 'Patient Number',
    y = 'Post-Surgical Symptom Score (Lower is Better)',
    title = 'Post-Surgical Symptom Score by Patient and Doctor (Randomized)'
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 14, face = 'bold', hjust = 0.5, 
...
@fig-plot-outcomes-severity presents a scatter plot showing post-surgical outcomes as a function of initial patient severity. This figure demonstrates the power of stratification, with the x-axis showing initial patient severity and the y-axis showing post-surgical symptom score (where lower is better). The scatter points are colored and marked by doctor using the same scheme as previous figures, with mean horizontal lines for each doctor. A rectangular overlay (shaded region) highlights severity between -1 and 1, and text annotations for means are positioned appropriately. This figure reveals the confounding by showing how severity affects both surgeon assignment and outcomes.
...
# Separate observational data by doctor and calculate means
...
# Create the plot
p <- ggplot(patients_df, aes(x = severity, y = post_surgical_score, 
                            color = doctor_name, shape = doctor_name)) +
  ...
  geom_point(size = 3, alpha = 0.7) +
  geom_hline(yintercept = dreamy_mean_sev, color = '#4E79A7', 
            linetype = 'dashed', linewidth = 1, alpha = 0.8) +
  geom_hline(yintercept = duck_mean_sev, color = '#E15759', 
            linetype = 'dashed', linewidth = 1, alpha = 0.8) +
  scale_color_manual(
    name = 'Doctor',
    values = c('Doc Dreamy' = '#4E79A7', 'Doc Duck' = '#E15759'),
  ...
  annotate('text', x = max(patients_df$severity) * 0.98, y = dreamy_mean_sev,
           label = paste0('○ ', sprintf('%.2f', dreamy_mean_sev)),
           color = '#4E79A7', size = 3.5, fontface = 'bold',
           hjust = 1, vjust = 0.5) +
  annotate('text', x = max(patients_df$severity) * 0.98, y = duck_mean_sev,
           label = paste0('△ ', sprintf('%.2f', duck_mean_sev)),
           color = '#E15759', size = 3.5, fontface = 'bold',
           hjust = 1, vjust = 0.5) +
  labs(
...
## Conclusion: Lessons for Causal Inference
